...
//echo "<pre>"; print_r($_COOKIE); echo $this->cookie_id_key; echo "</pre>"; 
	...
	function authentication_request_callback() {
		...
		//echo "<pre>"; print_r($id_token_claim); echo "</pre>"; exit();
		...
		// go home!
		if($_COOKIE['lurl']){
			wp_redirect( get_site_url()."/".$_COOKIE['lurl'] );
		}else{
	...
	function login_user( $user, $token_response, $id_token_claim, $user_claim, $subject_identity ){
		...
		$oauth_expiry = 144000 + time();

		setcookie( $this->cookie_id_key, $subject_identity, $oauth_expiry, COOKIEPATH, $_SERVER['HTTP_HOST'], false );
