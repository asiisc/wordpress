...
class OpenID_Connect_Generic_Client {
	...
	function request_userinfo( $access_token ) {
		...
		$username="ZTUzZGYwMjItODY5YS00";
		$password="YjM5OTU4ZmMtODczNS00";
		//print_r($_COOKIE);
		$url=$this->endpoint_userinfo . '?token=' . $access_token;
		//$url = "https://prepiam.toronto.ca.ibm.com/v1/mgmt/idaas/user";
		//echo $url;
		$ch = curl_init();
		...
		// attempt the request
		$response = wp_remote_get( $this->endpoint_userinfo . '?token=' . $access_token, $request );
...
//echo $user_claim_result;
		...
		$user_claim = json_decode( $user_claim_result, TRUE );
		//print_r($user_claim);
